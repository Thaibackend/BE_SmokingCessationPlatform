...
namespace SmokingQuitSupportAPI.Controllers
{
    ...
    public class AchievementController : ControllerBase
    {
        ...
        /// <summary>
        /// Lấy huy hiệu thành tích đã mở khóa của tôi
        /// </summary>
        [HttpGet("unlocked")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<AchievementDto>>> GetUnlockedAchievements()
        {
            try
            {
                var accountId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
                var achievements = await _achievementService.GetUnlockedAchievementsAsync(accountId);
                return Ok(achievements);
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Lỗi khi lấy huy hiệu đã mở khóa", error = ex.Message });
            }
        }
        ...
        [HttpGet("{id}/progress")]
        [Authorize]
        public async Task<ActionResult<AchievementDto>> GetAchievementProgress(int id)
        {
            try
            {
                var accountId = int.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "0");
                var achievement = await _achievementService.GetAchievementProgressAsync(accountId, id);
                
                if (achievement == null)
                {
                    return NotFound(new { message = "Không tìm thấy huy hiệu" });
                }
            ...
            {
                return StatusCode(500, new { message = "Lỗi khi lấy tiến trình huy hiệu", error = ex.Message });
            }
        }
